...
| 24 | 2026-02-20 | **Desktop app bug fixes + tab reorganization**: Fixed 52-second load delay (split init: core runs immediately, CDN-dependent deferred to window.load), loadSavedReports ReferenceError, ninja-keys blocking DOMContentLoaded (dynamic import), About modal stuck visible, driver.js auto-tour on every launch (removed localStorage trigger), AI schema suggestion timeout (added rule-based fallback). **Tab reorganization**: 6 primary workbench tabs (Research→Process→Review→Analysis→Intelligence→Export), 4 legacy tabs (Companies/Taxonomy/Map/Canvas) in Tools dropdown, Settings as gear icon. Refactored showTab() from index-based to data-tab attribute lookup. Updated keyboard shortcuts (1-6), command palettes, tours, desktop.py View menu. Default tab changed from Companies to Research. 1517 tests passing. | ✅ Complete |
| 26 | 2026-02-21 | **MCP Integration Tier 1+2**: MCP client (`core/mcp_client.py`) with 7 API wrappers (HN, DuckDuckGo, Cloudflare, USPTO, SEC, Companies House, Wikipedia) + SQLite cache. Enrichment orchestrator (`core/mcp_enrichment.py`) with 7 adapters, context matching, staleness checks. Enrichment blueprint with 5 endpoints. 4 new monitor types (hackernews, news_search, traffic, patent). 2 new lens endpoints (enriched-matrix, market-map). Frontend enrichment UI. 101 new tests, 1618 total. | ✅ Complete |
